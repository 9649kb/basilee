...
const DEFAULT_CONTENT = `01. Éditeur du site
Le présent site est édité par Basile KADJOLO, Entrepreneur individuel basé à Lomé, Togo.
Activité : Prestations de services digitaux et Formations.

02. Propriété Intellectuelle
L'ensemble des contenus (textes, graphismes, logos, vidéos) présents sur ce site est la propriété exclusive de Basile Kadjolo, sauf mention contraire. Toute reproduction, même partielle, est strictement interdite sans autorisation préalable.

03. Hébergement
Ce site est une application web moderne hébergée sur des serveurs sécurisés garantissant une haute disponibilité pour l'Afrique et l'international.`;
...
const LegalNoticeModal: React.FC<LegalNoticeModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('basile_legal_notice');
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('basile_legal_notice', tempContent);
    setIsEditing(false);
  };
    ...
    <div className="fixed inset-0 z-[800] flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-[2.5rem] p-8 md:p-12 max-w-2xl w-full relative z-10 shadow-2xl animate-fade-in-up max-h-[85vh] overflow-y-auto custom-scrollbar text-left text-slate-600 text-sm leading-relaxed">
        ...
        <div className="mb-10 flex justify-between items-start">
          <div>
            <span className="text-blue-600 font-black tracking-widest uppercase text-[10px] bg-blue-50 px-3 py-1 rounded-full mb-4 inline-block">Information</span>
            <h2 className="text-3xl font-black text-slate-900 leading-tight">Mentions <span className="text-blue-600">Légales</span></h2>
          </div>
          {isAdmin && !isEditing && (
